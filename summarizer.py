...
def main():
    path = 'C:\\Users\\Arpit\\PycharmProjects\\summarizer\\corpus\\'
    for x in range(1, 101):
        # fileName = path + 'machine_output\\tokenized' + str(x) + ".txt"

        initFileName = path + 'Test\\input' + str(x) + ".txt"
        initSentences = []
        with open(initFileName, 'r', encoding='utf-8') as initDoc:
            ...
            initSentences = initSentences[0:-1]
...
def textrankTfIdf(document):
    ...
    # print(document[len(document)-2])
    # sentences = document[0: -1]
    sentences = document
    bow_matrix = CountVectorizer().fit_transform(sentences)
    normalized = TfidfTransformer(norm='l2', use_idf=True, use_bm25idf=True, smooth_idf=True,
                 delta_idf=False, sublinear_tf=False, bm25_tf=True, use_bm25_epsilon=True, epsilon=0.25).fit_transform(bow_matrix)
